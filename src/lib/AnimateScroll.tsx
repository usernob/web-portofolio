...
const AnimateScroll = ({className: classNameProps, addClass, removeClass, children}: Props) => {
    ...
    const handleScroll = useCallback(() => {
        if(!isBellowViewport()) {
            ...
            removeClass?.split(" ").forEach((value) => {
                setClasses(classes?.replace(value, ''));
            })
        }
        else {
            setNewClass('');
    ...
    const listener = useMemo(() =>
        throttle(() => { if (node.current) handleScroll(); }, 50),
        [handleScroll]
    );
